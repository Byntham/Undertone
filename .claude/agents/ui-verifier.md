...
- The window's `on_save` callback MUST be a no-op (`lambda _cfg: None`) — NEVER `config.save_config` or anything that reaches it, and NEVER construct `main.App` (its settings callback writes the user's REAL `%APPDATA%\Undertone\config.json`; a verification run once flipped the owner's live providers this way). The settings UI autosaves on combo/toggle changes, including programmatic ones, so any real save path WILL fire. Make the harness process exit deterministically (QTimer-driven `app.quit()`); a leftover live window can fire saves long after your checks finish.
