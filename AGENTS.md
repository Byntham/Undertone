...
- Tests live in `tests/` (plain assert scripts, no pytest): `test_textproc.py`, `test_pipeline.py` (context-to-final-text wiring; no desktop), `test_gestures.py`, `test_providers.py` (mocked, keyless), `test_localstt.py`, `test_localllm.py`, `test_caretctx_unit.py` (desktop-independent range logic), `test_caretctx.py` (live Windows UIA), `test_cleanup.py` (live xAI), and the manual `test_e2e.py` / `test_focus_return.py`, plus the settings behavior suite `spikes/qt_settings_behavior.py` and the perf gate `tests/perf_settingsui.py`. Routine verification uses `test_pipeline.py` + `test_caretctx_unit.py`, never the live desktop scripts. `test_e2e.py` and `test_focus_return.py` are opt-in (`UNDERTONE_DESKTOP_E2E=1` / `UNDERTONE_FOCUS_E2E=1`) because they necessarily take foreground focus; run them only after the user confirms the desktop will remain idle. They use disposable WPF targets tied to kill-on-close jobs, fake `transcribe`, and drive the f13 hotkey. Key injection MUST come from a separate process (`keyboard` hides its own injected events from same-process hooks via `is_replaying`). `test_caretctx.py` likewise steals focus and is manual-only. User input during any live test invalidates the run; treat that as a harness failure, not an app failure.
...
**Providers**: STT (`provider`) and AI cleanup (`cleanup_provider`) are independently selectable across xai/openai/openrouter, and BOTH additionally offer `local` (see below); keys live in per-provider config fields (`config.KEY_FIELDS`/`provider_key`), model overrides in `stt_model`/`cleanup_model` (empty = `DEFAULT_STT_MODELS`/`DEFAULT_CLEANUP_MODELS`). Vocabulary biasing is xAI-ONLY (`keyterm` multipart fields, docs-pinned format; gated by the `stt_vocab_hints` toggle, which empties the list before `transcribe()`): prompt-based biasing for the other providers was removed 2026-07-17 — whisper-style models condition on the prompt as prior transcript and echoed mangled term lists into transcripts on marginal audio, regardless of prompt wording; don't reintroduce (the dictionary still reaches those providers' output via corrections + AI cleanup). Cleanup providers all speak the OpenAI chat dialect; only `cleanup.API_URLS` differ. Request shapes are pinned by `tests/test_providers.py` (mocked, no keys needed).

**Local STT** (`localstt.py`, provider id `local`, keyless — `transcribe()` skips the key guard and `KEY_FIELDS` deliberately has no entry): a whisper.cpp `whisper-server.exe` child on a loopback port, POSTed the same multipart dialect (no vocabulary `prompt` — see Providers). Everything installs on demand into `%LOCALAPPDATA%\Undertone` (never roaming) from a sha256-pinned manifest: CPU runtime always (tiny, doubles as fallback), CUDA runtime too when `nvcuda.dll` is present, the ggml model + a Silero VAD model. VAD is why the banned-from-cloud whisper-large-v3-turbo is safe here — the server returns empty text on silence instead of hallucinating; the VAD model (silero-v6.2.0) comes from `ggml-org/whisper-vad`, whisper.cpp's own artifact repo (the same source its `models/download-vad-model.sh` uses — verified official, don't "fix" it to another mirror). If the CUDA server fails to start, `runtime.json` gets `cuda_disabled` and CPU takes over (delete the file to retry GPU). Residency controls are deliberately decoupled: the card's Load/Eject button is a pure runtime action, while the UNIFIED pair `local_loaded` ("load on startup") and `local_idle_minutes` (0 = never) drives BOTH local engines — startup warming is additionally gated per engine on its provider actually being `local` (STT: `provider`; cleanup: `cleanup_provider` + `ai_cleanup`), and the idle auto-eject timer runs inside each engine module (timer body re-checks under the lock, gestures-style; legacy per-engine keys fold via `config._fold_legacy_local`). Dictating while ejected auto-loads for the session with a "Loading the local model…" pill and flips nothing. A kill-on-close job object ties the child to the app through Task Manager kills; `App._quit` also calls `localstt.shutdown()`. Settings drives install/load/eject on worker threads via the `_queue`/`_drain` pattern with per-percent progress. The `stt_models["local"]` override is a ggml FILENAME in the models dir. Offline logic is pinned by `tests/test_localstt.py`.

**Local cleanup** (`localllm.py`, cleanup provider id `local`, keyless): the STT twin for AI cleanup — a llama.cpp `llama-server.exe` child (pinned release b10064; CUDA zip is split, the cudart DLLs come as a second zip merged into the same build dir) running Unsloth's `Qwen3-4B-Instruct-2507-Q4_K_M.gguf` (non-reasoning — a thinking model would blow the latency budget). llama-server speaks the OpenAI chat dialect natively including grammar-enforced `json_schema`, so `cleanup.py` sends its normal body to `base_url() + /v1/chat/completions` with no auth header. Same manifest/install/CUDA-fallback/job-object/idle-eject machinery as localstt — the generic parts live in `localproc.py` (shared by both; don't re-duplicate) — with one extra rule: **cleanup never load-blocks a dictation**. The pipeline only ever calls `localllm.base_url()` (running server or None — and, like `is_loaded()`/`active_build()` in both engines, it uses a NON-blocking lock acquire, because `ensure_ready` holds `_LOCK` across the whole model load and a contended read must report "not ready", not stall the pipeline or freeze the settings poll on the Qt main thread); on a miss, `cleanup.cleanup()`'s local branch fires single-flight `load_async()` and that dictation falls back to rules while the model warms for the next one (`ensure_ready` blocks and is for Settings workers/tests only). State files are `llm-runtime.json`/`llm-server.log`; build dirs `runtime/llm-cpu|llm-cuda`; the model shares `models/` with STT. Residency follows the unified `local_loaded`/`local_idle_minutes` pair (see Local STT); `cleanup_models["local"]` = GGUF filename. GPU cleanup is ~0.2 s/dictation; CPU is 0.8–2.6 s and often loses to the 2.5 s budget — that's the accepted tradeoff (silent fallback), tunable via the dev-mode-only `cleanup_timeout` field in Providers → Advanced (config default 2.5; the same dev-mode Advanced block also has a `cleanup_prompt` editor overriding `cleanup.SYSTEM_PROMPT` ("" = default) with named saves in `cleanup_prompts` {name: text} — picking a save loads it and makes it live; Save updates the selected save; both knobs apply to ALL cleanup providers, not just local). Pinned by `tests/test_localllm.py` + the local shape test in `tests/test_providers.py`.

**Smart formatting context** comes from three tiers, tried in order: UIA TextPattern2/GetCaretRange (dedicated COM worker thread, 150ms timeout — WPF/browsers; ValuePattern or LegacyIAccessible may additionally prove that a field is exactly empty, including Chromium contenteditables whose TextPattern is contaminated with surrounding UI and ends at an object-replacement marker), a Win32 `EM_GETSEL`+`WM_GETTEXT` fallback via `SendMessageTimeout` (classic Edit/RichEdit controls, e.g. Notepad; bails at ≥64k chars because EM_GETSEL packs positions into LO/HIWORD), and insertion memory (what Undertone itself last pasted, valid only while the same window is focused and its paste-start input-generation token still matches — this avoids losing a key/click that races post-paste bookkeeping). UIA and Win32 return bounded text on BOTH sides of the caret (or outside a selection being replaced), so `textproc.finalize()` handles both the leading and trailing insertion seams; right-side text stays local and is never included in the AI cleanup request. Empty-only Value/Legacy checks return no text and therefore add no AI-cleanup privacy exposure. Insertion memory supplies the left side only. Terminals/Google Docs/most Electron get memory only. Unknown context on either side = do nothing risky on that side. (A fix-window auto-learning flow existed briefly and was removed 2026-07-11 — the owner prefers manual dictionary entries; don't reintroduce without asking.)

**AI cleanup** (`cleanup.py`, config `ai_cleanup`/`cleanup_model`) is a hybrid: the grok chat model handles the transcript body (fillers, false starts, dictionary variants, mid-sentence casing) via structured output at temperature 0, while `textproc.finalize(model_cased=True)` re-applies the exact dictionary and re-decides both insertion seams (spacing + sentence-start capital) deterministically — both pipeline branches funnel through `finalize()` so a cleanup timeout can't change dictionary behavior — the probe showed each layer fails where the other is reliable. Any HTTP error/timeout (1.5s connect, 2.5s read) returns None → silent fallback to `format_transcript`. `_drop_echoed_context` cuts a ≥4-char context tail the model may echo. Dictionary reaches three stations: STT `keyterm` hints (xAI only), exact regex replace, and the model (fuzzy variants). Privacy: left ctx + window title go to xAI when enabled; right ctx stays local — disclosed in the toggle hint.
...
- NEVER call `setStyleSheet` on a container that has attribute-styled descendants: a local sheet on any ancestor silently drops the window QSS's attribute-selector rules (`[variant="accent"]`, `[hover="true"]`) for everything beneath it — type/ID selectors keep cascading, which makes the breakage look random (this rendered History's accent Retry button invisible and killed row hover). Style containers via objectName + a window-QSS rule instead. Related trap: a background in a generic `QWidget {}` rule paints EVERY plain wrapper widget (polish sets WA_StyledBackground on any match), producing subtle wrong-tone rectangles on cards/panels — that's why the window background is scoped to `QWidget#settingsWindow` and wrappers stay unpainted. Two more: QSS silently DROPS a border-radius larger than half the widget's height (24px borderless accent buttons rendered square while 26px bordered neutrals were pills — keep pill radii ≥2px under height/2, and accent buttons carry a transparent border for geometry parity); and `QScrollArea.setWidget()` flips the content widget's autoFillBackground ON, which under Windows dark mode paints the Qt PALETTE color (#1e1e1e), not the theme — flip it back off (`_wrap_scroll`, `list_panel`).
- All drop-downs go through the `Combo` class, not raw QComboBox — it ignores wheel events (scrolling the page must not change settings), sets the styled item delegate `::item` QSS needs, styles the popup via `POPUP_QSS` (the popup is a top-level window the main QSS can't reach), rounds it via DWM corner preference (the popup stays OPAQUE — a translucent popup left unpainted container strips flashing whatever was behind it), and enables the popup scrollbar (stock combos hard-clip over-tall lists).
- Verify COLORS and popup/compositor effects against the SCREEN (`QScreen.grabWindow` with the window shown and raised, pixel-sampled), not `widget.grab()` — offscreen grabs render the widget in isolation and miss palette fallbacks (dark-mode #1e1e1e leaks), DWM effects, and translucency artifacts; several defects passed grab()-based verification while visibly broken live.
