...
- Tests live in `tests/` (plain assert scripts, no pytest): `test_textproc.py`, `test_pipeline.py` (context-to-final-text wiring; no desktop), `test_gestures.py`, `test_providers.py` (mocked, keyless), `test_localstt.py`, `test_localllm.py`, `test_caretctx_unit.py` (desktop-independent range logic), `test_caretctx.py` (live Windows UIA), `test_cleanup.py` (live xAI), and the manual `test_e2e.py`, plus the settings behavior suite `spikes/qt_settings_behavior.py` and the perf gate `tests/perf_settingsui.py`. Routine verification uses `test_pipeline.py` + `test_caretctx_unit.py`, never the live desktop scripts. `test_e2e.py` is opt-in (`UNDERTONE_DESKTOP_E2E=1`) because it necessarily takes foreground focus; run it only after the user confirms the desktop will remain idle. It uses a disposable WPF target tied to a kill-on-close job, fakes `transcribe`, and drives the f13 hotkey. Key injection MUST come from a separate process (`keyboard` hides its own injected events from same-process hooks via `is_replaying`). `test_caretctx.py` likewise steals focus and is manual-only. User input during either live test invalidates the run; treat that as a harness failure, not an app failure.
