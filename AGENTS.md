...
- UI verification: instantiate `settingsui.SettingsWindow` / `overlay.Overlay` directly on a withdrawn `tk.Tk()` with a fake `level_getter`, call their show methods, capture with `PIL.ImageGrab` after `root.after` delays (PrintWindow-based capture avoids occlusion by other apps; dropdown popups are separate Toplevels and need their own grab). Prefer delegating capture+judgment to the `ui-verifier` agent (`.claude/agents/ui-verifier.md`) — the main session should consume verdicts, not screenshots.
...
**The settings window is canvas-rendered, not a widget tree.** Every Tk widget is a real child HWND on Windows (~60ms/step to revalidate a full settings tree during a resize — the old window needed snapshot-freeze hacks). `settingsui.SettingsWindow` instead draws everything as canvas items on two canvases (fixed sidebar + scrollable content) via the `canvasui.py` framework, so resizes relayout LIVE every `<Configure>` (~11-15ms/step). Its invariants:
- NO persistent embedded native widgets (`create_window` costs ~1.2ms/step EACH): text editing goes through the Scene's single shared Entry overlay, materialized only over the field being edited.
- No Pillow/ImageTk work in the relayout path — rounded corners are cached backdrop-baked OPAQUE corner-cap images (alpha compositing is Tk's slow path) + plain rectangles for edges/interiors; `tests/perf_*` assert zero `CAP_CACHE` misses during storms.
- Relayout is incremental: `Scene._coords`/`_itemconfigure` dirty-check against last-applied values and batch through one Tcl `eval` (string values must go through the `_TCL_WORD` escaping — user text once broke the batching).
- Long lists (History, Dictionary) virtualize via `ListView` — item count stays bounded (~200/section) or resize cost creeps back up.
- Perf gates: `tests/perf_canvasui.py` (framework stress pane, <18ms/step median) and `tests/perf_settingsui.py` (every real section, <18ms/step, idempotent relayout, History item budget). Run both after touching canvasui/settingsui layout code.
`ui.py` retains only the tray (`create_tray`, tray/recording icons), `pretty_combo`, nav glyphs, and shared tables (LANGUAGES/PROVIDERS_UI/SECTIONS…) — both main.py and settingsui import from it. The One-Dark slate ladder lives in `theme.py` (single source for ui/settingsui/canvasui/overlay). Windows polish via ctypes: dark title bar (DWM attribute 20/19), crisp taskbar/title-bar icons via `WM_SETICON` with exact-size frames from `assets/icon.ico` (Tk's `iconbitmap` alone registers only the 16px frame, which Windows scales blurrily).

**High-DPI**: `theme.init_dpi()` (called at the top of main.py, before any Tk/pixel work) makes the process system-DPI aware; every pixel measure in ui.py/settingsui.py/canvasui.py/overlay.py goes through `theme.sc()`. Point-sized Tk fonts scale on their own — new pixel constants must use `sc()` or they'll be tiny on scaled displays. overlay.py scales its constants at import time, which is why init_dpi must run before it is imported.
...
Other Windows realities: NEVER set `.argtypes`/`.restype` on `ctypes.windll.*` functions — that cache is process-wide shared, and one module's prototypes poison another's calls (a `CreateDIBSection` prototype set in ui.py silently killed every overlay pill render); use a private `ctypes.WinDLL("...")` instance whenever you set prototypes. Pasting into elevated apps requires Undertone to be elevated too; injected key events are filtered from the hotkey hook by their missing scan code; recordings under ~0.3s are discarded (`MIN_AUDIO_BYTES`), and speechless ones are never sent to STT (`main._speech_windows`: needs ≥3 30ms windows at RMS ≥250, edges trimmed where the hotkey's own clack lives — a raw peak threshold proved fragile against the owner's ~360-peak noise floor) — Whisper-family models hallucinate on silence (echoing the vocabulary prompt, which `transcriber._looks_like_prompt_echo` additionally filters to empty); `transcriber.PROVIDERS` is the extension point for additional STT providers.
