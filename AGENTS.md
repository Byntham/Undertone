...
**Styling is hand-rolled**, not ttk: plain tk widgets, the One-Dark-derived slate ladder lives in `theme.py` (single source for `ui.py` and `overlay.py`), Pillow-rendered imagery. The settings window is grouped setting cards (`_card`/`_row_card`/`_toggle_card`) with pill-shaped `RoundButton`s (Pillow background + Tk `compound="center"` text). Windows polish via ctypes: dark title bar (DWM attribute 20/19), crisp taskbar/title-bar icons via `WM_SETICON` with exact-size frames from `assets/icon.ico` (Tk's `iconbitmap` alone registers only the 16px frame, which Windows scales blurrily).
